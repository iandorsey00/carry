...
Differential Equations is Carry's first university-level course. Its thirteen-lesson sequence covers foundations, first-order equations, numerical methods, second-order equations, systems, Laplace transforms, and series solutions. Course labs include a draggable initial condition on a slope field, a step-size-controlled Euler approximation, and an autonomous phase line. Separable Equations, Linear First-Order Equations, and Homogeneous Linear Equations now use staged solving workspaces with guided focus, authored equivalent forms, contextual hints, canonical MathML results, initial-value problems, and less constrained Practice modes.
